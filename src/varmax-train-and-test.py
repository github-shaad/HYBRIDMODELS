...
PlotSave().multiPlot(TICKERS, np.exp(test_prices_log), np.exp(var_test_predicted_prices_log), "VARMAX", "model")
StorageManager.store_predictions(np.exp(var_test_predicted_prices_log), "model", MODEL_NAME)
Metrics.save_model_metrics(MODEL_NAME, np.exp(test_prices_log), np.exp(var_test_predicted_prices_log))
test_prices_true = np.exp(test_prices_log)
test_prices_pred = np.exp(var_test_predicted_prices_log)

signal_matrix, tradable_prices = generate_signals(test_prices_true, test_prices_pred)
...
param_grid = {
            'hidden_layer_sizes':[(32,), (32,32), (64,)],   
            'max_iter':[100],              
            'alpha':[0.1, 0.5],                
...
backtest = RankedBackTesting2(signal_matrix, tradable_prices, 3,3, 0.001)
equity_curve, drawdown_curve = backtest.equityDrawdownCurve(10_000)
Metrics.save_portfolio_metrics(MODEL_NAME, equity_curve)
btPlot = BacktestEquityDrawdownPlot(MODEL_NAME)
btPlot.plot({MODEL_NAME:(equity_curve, drawdown_curve)})
StorageManager.store_figures(btPlot, "portfolio", f"{MODEL_NAME}_ranked")
"""